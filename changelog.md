...
* Nova extensão crome na loja google para facilitar instalação - Botão conectar web separado QRCODE. Fica la botao abre qrcode "Whatsapp WEB" Detecta se instalação ta instalada caso instalada faz quase todo processo so cliente conectar no whatsapp web. E se não tiver abre instruções como instalar. Se tiver ja conectado no web e extensão instalada clicar esperar faz tudo automatico.
...
* Adicionado horário de início e término para campanhas. g
...
* Reformulada a contagem do dashboard para usuários de atendimento, refletindo corretamente os dados de tickets finalizados pelo usuário. Anteriormente, o sistema considerava apenas acesso ao ticket, gerando dados incorretos
* Opção para ocultar o Kanban Pro e o financeiro da empresa
* Contagem de cliques na página de indicação
* Novo cadastro exclusivo para afiliados gerenciarem suas indicações
* Financeiro agora possui modal para o cliente consultar o espaço em uso
* Período de tolerância para pagamento
* Opção de fixar ticket
* Opção de arquivar ticket — ao receber nova mensagem, o ticket retorna automaticamente para “Abertos”
* Importação de contatos de grupos
* Opções de importação agora exibem no frontend a quantidade de contatos importados e atualizados
* Novos endpoints da API Admin
* Quebra de linha extra na assinatura
* Troca de fonte na tela de atendimento
* Avaliação voluntária (não bloqueia novo atendimento)
* Solicitação de feedback após avaliação
* Cadastro de período de feriados e exclusão em massa de marcações
* Configuração para definir o número máximo de vezes que a mensagem de horário de atendimento deve ser enviada ao cliente. Configuração movida para a tela de horário de atendimento
* Nova configuração para não enviar mensagem de horário de atendimento em tickets abertos
* Bot agora sempre é acionado, mesmo fora do horário de atendimento. O comportamento deve ser tratado diretamente no bot
* Chatbot Interno com simulação básica para testes durante a construção do fluxo
* Chatbot Interno com nova opção de transferência para fila + usuário
* APIs com limite de resposta em até 24 horas. Após o prazo, o envio pela tela de atendimento é bloqueado, obrigando o uso de template
* Tela de atendimento agora carrega mais mensagens apenas da aba selecionada, em vez de carregar todas as abas
* Tela de atendimento com filtro por usuário quando “Visualizar Todos Tickets (Admin)” estiver desativado
* Notificação e atualização quando ativa informativo painel saas
* Encaminhamento de chat interno para WhatsApp
* Exclusão de empresa agora também remove os arquivos. O processo pode ficar mais lento caso existam muitos arquivos
* Novo sistema de encaminhamento de mensagens na tela de atendimento. Agora será necessário selecionar canal e fila. O encaminhamento só será permitido se o ticket pertencer ao usuário que está realizando a ação e tiver acesso canal para abrir ticket
* Opção para renomear arquivos enviados na base de recepção inteligente
* Ajustes tela atendimento e outros locais frontend para tentar trazer melhor desempenho

### 🚀 Versão 2.19.3 Final

* bug - modo dark limite mensagem json
...
* bug - contato enviado em modo dark Sistema tenta reconexão envio mensagens caso canal aparece como desconectado
...
* Necessário reinstalar redis - https://doc.whazing.com.br/solucao-de-problemas/reinstalar\_redis
...
Agora deve definir hora permitida para envio. Hora de início não será mais usada; será usada a hora em que for adicionado o túnel, validando as horas permitidas. Caso esteja fora da hora permitida, será enviada no próximo horário permitido.
...
**Mensagens rápidas por fila** Deixar tudo mais organizado por setores de atendimento.

**Mensagens rápidas** Definir cores para atalhos.

**Mensagens de despedida por fila** Deixar tudo mais organizado por setores de atendimento.
...
Não será possível cadastrar novos. Os que já estão cadastrados serão mantidos.
...
Integrações ia separadas foram descontinuadas, deve ser usar recepção inteligente - As que estão configuradas continua funcionando não possivel criar mais novas (Pretendo trazer melhorias maiores na recepção inteligente, hoje como substitui modelo não acho viavel continuar mantendo)
...
🐞 \*_bug_- – criando índices únicos para informações adicionais dos contatos, para não duplicar. Caso seja detectado repetidas, será apagada as mais antigas, mantida somente a mais recente criada.

🐞 \*_bug_- – aumento da quantidade de caracteres nas colunas da tabela **"PushSubscriptions"**, que causava erro em alguns dispositivos ao cadastrar push devido à quantidade de caracteres.

🐞 \*_bug_- – validação do campo tempo de fechamento do ticket bot. Deixar esse campo em branco faz com que nenhum bot feche atendimento automático.

🐞 \*_bug_- – listar último ticket fechado. Quando tem mais de um canal, só vai listar o ticket mais recente, não localizando conversa de outros canais.

🐞 \*_bug_- – reações em grupos sumirem.

🐞 \*_bug_- – controle de zoom atrás de mensagens rápidas.

🐞 \*_bug_- – segunda via de boleto Assas limitada aos próximos 6 meses, para não cortar boletos.

🐞 \*_bug_- – não exige mais telefone para poder editar contatos como hubs.

***

📝 **módulo avaliação**

3 mensagens avaliação erradas volta abrir ticket Mensagens inválidas ficam registradas no ticket
...
* Atribuir tarefas a equipes Configuração para usuário comum poder atribuir tarefas a equipes que ele não faz parte Emite mensagem no chat interno nova tarefa

***

🔄 **Sincronização mensagem API Plus e Wuzapi**

Vai solicitar JID e LID. Então contagem total de mensagens às vezes pode dar errado. Antes era solicitado somente JID, mas quando mensagem chegava como LID essas mensagens ficavam fora da sincronização. Achei melhor alterar.

***

🔌 **Wuzapi**

Mudanças para compatibilidade com nova versão. Necessário atualizar WUAZAPI:

```
curl -sSL wuzapi.whazing.com.br | sudo bash
```

Suporte lista, botão, chave Pix, copy, call (somente chave, sem valor). (Somente minha versão, original não tem suporte) Sem garantia, use por sua conta e risco.
...
Suporte envio botão Pix, API, tela atendimento, Typebot, Bot Interno. (para tela de atendimento cadastrar chaves previamente na tela canais)
...
2 novas opções ferramentas canais: 1 – exibir informações do canal 2 – atualizar URL webhook

Renderizar templates enviados via API ou campanha. Suporte conexão TechProvider com embed.

Caso mensagem X (-1) tenha retorno ACK, consultar informações de retorno. Caso tenha, vai aparecer onde tem opção responder.

Valida dados JSON botão e lista para evitar erros de envio. Template hello\_word retirado da lista.
...
Suporte usar proxy configurado no `.env`. (proxy do `.env` não valida se está funcionando)

[https://doc.whazing.com.br/docs/configuracoes-vps-e-whazing/proxytodascontas\_whatsapp](https://doc.whazing.com.br/docs/configuracoes-vps-e-whazing/proxytodascontas_whatsapp)

Prioridade proxy banco, se não tiver vai tentar do `.env`.

`PROXY_URL_VALID=true` Com essa configuração ele vai validar proxy. Caso falhar, não vai usar.

Vocês devem sempre verificar logs, pois como o canal conecta pode parecer falso positivo (vocês acham que está usando proxy, mas não).

`PROXY_URL_VALID` somente para Baileys e Wuzapi. A API Plus tem regras próprias, se tiver erro de proxy usa deles automaticamente.
...
Opção desativar bot interno daquele ticket quando abre atendimento que está no meio do bot (vai ficar aqueles ícones de baixo editar lado direito).

Filtro admin usuário exibe foto se usuário está online. Filtro cliente aguardando resposta (somente filtra abertos).

➕ Colocado atalho adicionar anotação no ticket também.

📝 **Anotação Ticket**

Possibilidade marcar usuários. Ao marcar, eles recebem mensagem no chat interno com anotação feita.
...
Exibe informações na lista de tickets se está bot, usa filtro bot. Possível selecionar se deve usar botão, lista e botão copy, tornando mais dinâmico.

Integração 2ª via boleto provedores. Caso não consiga fazer desbloqueio de confiança, não transfere mais atendimento direto da mensagem: "Desculpe! 😕 Não consegui completar o desbloqueio da sua conexão."
...
No cadastro opção **"Validar se o número possui WhatsApp"**. Permite desativar validação contatos permitindo cadastrar mesmo não tendo WhatsApp. Cuidado – isso pode causar problemas se iniciar conversa com esses contatos.
...
🎨 Muitas mudanças no layout, não vou especificar. ⚠️ Muitas mudanças, algumas esqueci de documentar.

***

🔧 **Canais HUB**

Liberado campo para editar token do canal, facilitando caso de manutenção. Ao reiniciar canais, ele força configurar webhook para casos de perda.
...
Configuração Admin Wuzapi. Gera token automaticamente ao cadastrar novo canal Plus ou fazer migração.

🔑 **Gerenciamento de Tokens Plus**

Cadastra tokens adquiridos dos servers do Whazing. Cliente cadastrar novo canal ou migrar vai usar um dos tokens disponíveis. Caso não encontre, vai dar erro.
...
* Corrigido erro ao \*_exportar relatório de tickets_- em Excel.
...
* Corrigido problema de \*_cor do texto em modo escuro_- nos títulos das colunas.

**⚙️ Melhorias Gerais**

* Adicionada lógica de **retry no download de arquivos (API Plus)**.
* O sistema agora \*_respeita o limite de download configurado_- no painel SaaS.
* Adicionada **configuração para exibir reações como respostas**.
* \*_Chamadas recebidas_- agora abrem automaticamente um ticket e **acionam integrações e bots**.
...
* Nova API **mais leve e independente**, executada em serviço separado, \*_melhorando o desempenho geral_- do sistema.
* Necessário \*_instalar o Docker separado_- e \*_atualizar o sistema_- — instruções disponíveis na documentação.
* Assim como a API Baileys, o \*_Wuzapi também obedece ao limite de download configurado_- no painel SaaS.
...
* Tickets originados de \*_integrações, IA ou TypeBot_- seguem a regra da configuração _“Não visualizar Tickets no ChatBot”_.
* No **editor de ChatBot**, ao clicar em “Voltar”, o sistema perguntará se deseja salvar as alterações.
* Novo comando no \*_TypeBot_- para **transferir ChatBot interno**.
* \*\*Nova API:\*- endpoint para **transferência de ChatBot interno**.
* Na listagem de ChatBots, agora é exibido o \*_ID do ChatBot_- para facilitar uso via API ou TypeBot.
...
* Na tela de atendimento, \*_contatos recebidos_- agora possuem **botão direto para abrir ticket**.

**🔔 Notificações**

* Nova configuração para \*_selecionar o som da notificação_- (7 opções de áudio ou modo silencioso).
...
* Adicionadas \*_badges de contagem_- de mensagens não lidas nas abas de usuários e grupos.
* Após enviar mensagem com _Enter_, o foco retorna automaticamente ao campo de texto para facilitar a digitação contínua.

**🧩 Tela de Atendimento**

* \*_Otimizada a listagem de tickets_- para melhor desempenho — recomenda-se testar com diferentes filtros e tipos de usuários, pois era uma área propensa a bugs.
* **Melhoria no carregamento das mensagens**, resultando em uma experiência mais fluida.

**💳 Plano Vencido**

* Usuários comuns \*_não visualizam mais valores ou mensalidades_- na área financeira quando o plano está vencido.
...
* Adicionada opção para \*_ocultar itens do menu lateral direito_- (apenas oculta, não remove).
* Nova função para \*_visualizar o espaço utilizado_- na pasta `public` por empresa.
* \*_Estatísticas por empresa_- agora disponíveis.
...
* Incluída \*_mensagem de erro_- em casos de falha de comunicação com o backend.
...
* Substituído o módulo de gravação áudio para \*_mensagens agendadas_- e **mensagens rápidas**.

**Integrações**

* \*\*Correção Assas (2ª via):\*- quando havia múltiplos boletos na lista, ocorria erro.
* \*_2ª via Assas, IXC e Hubsoft_- com API Plus agora utilizam **botão "Copiar e Colar"**.
...
* Integração com sistema \*_IXC_- para emissão de boletos e desbloqueio de confiança.
* Atualização do \*_Baileys_- - recomendado ler Novamente QRCODE.
...
\*_Atenção_- campanhas criadas que envia arquivos antes dessa versão não vai funcionar, deve alterar arquivo -
...
* Corrigida a \*_cor da legenda nos gráficos de pizza_- ao passar o mouse (hover)

**⚙️ Melhorias de Sistema**

* \*\*Otimização no backend:\*- remoção de trechos de código não utilizados
* \*_Melhoria visual_- na exibição de áudios na tela de atendimento
* \*_Zoom na imagem de perfil do contato_- na tela de atendimento
...
* Pressionar \*_Ctrl + Enter permite quebra de linha_- na tela de atendimento

**🤖 Integrações e Automações**

* Atendimentos feitos por \*\*automações (IA, Typebot)\*- agora marcam mensagens como lidas automaticamente
* Tickets com \*_integrações ativas_- (como Typebot ou IA) **não acionam validação de horário de atendimento**
...
* \*_Filtro por canal_- adicionado à dashboard (além do filtro por data)
...
* Validação de \*_horário de atendimento_- ajustada: se o responsável estiver online e o ticket aberto, aviso fora do horário não será enviado
* Implementado \*_cache no título da página_- para caso queda backend manter titulo
...
* Sistema passou a exigir \*_senhas mais complexas_- para maior segurança
