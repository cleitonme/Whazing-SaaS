...
Na tela de **Atendimento**, o atendente pode criar e acompanhar os agendamentos do cliente **sem sair da conversa**. O painel de contatos tem uma aba **Agenda** que mostra os horários daquele cliente e permite agendar em poucos cliques — o cliente já vem preenchido automaticamente.

## 📍 Onde encontrar

1. Abra um **atendimento** na tela de Atendimento.
2. À direita da conversa fica o **painel do cliente** (gaveta lateral). Se ele estiver fechado, abra pelo ícone no canto superior direito.
3. No topo do painel estão as abas: **Perfil · KANBAN · Atividade · Agenda**.
4. Clique em **Agenda**.
...
## 📋 O que aparece na aba Agenda

A aba mostra **todos os agendamentos do cliente** (de todos os calendários que você pode ver), divididos em duas listas:
...
1. Com o atendimento aberto, vá na aba **Agenda** do painel do cliente.
2. Clique no botão **"Agendar"** (canto superior direito da aba).
...
## ✏️ Gerenciando agendamentos existentes

Clique em qualquer item das listas **Próximos** ou **Anteriores**:

* A mesma janela abre em **modo de leitura**, com a etiqueta de **status** no topo.
* Clique em **Editar** para alterar dados (calendário, profissional, serviço, horários, observação) e **Salvar**.
* Use **Cancelar** para cancelar o agendamento (com confirmação) — veja [Cancelamento de agendamentos](cancelamento-de-agendamentos.md).
...
* **Cliente pedindo horário na conversa?** Você nem precisa sair do atendimento: abra a aba **Agenda** e clique em **Agendar** — em segundos o horário está marcado.
* **Cliente perguntou "que horário marquei mesmo?"** — a resposta está na lista da própria aba.
* **Atendimento encerrado sem agendamento?** Sem problema: o agendamento também pode ser criado depois pelo painel da [Agenda](como-usar-a-agenda.md) — o cliente é só buscar no campo **Cliente**.
